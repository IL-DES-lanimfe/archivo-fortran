...
C Septiembre 2006                                                    	C
C								     	C
C Adaptacion a Gaussian core model (GC)					C
C									C
C Agosto 2008								C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
CC
	Integer i,apro
	Double precision pi,dy,eps,Gder(Nk,2)
!	Double precision Rk(Nk),Sk(Nk)
CC
C funciones que se usan
CC
	Double precision Smax,SdkGCRPA
CC
...
CC
	...
	write(*,*)'Este es el caso del pot. de Yukawa atractivo'
        write(*,*)'parametro1:T(temperatura)'
	write(*,*)'parametro2:Densidad reducida'
	write(*,*)' '
...
CC
	...
	pi= dacos(-1.D0)
!	z=100.D0
	dpar2=0.05D0 !dabs(par2max-par2min)/dble(Npar2)
        open(8,file='cristalGC.dat') 
	dy=0.25D-2
...
!	 call msa(par1ant,parametro2,z,0.01D0,Nk,Rk,Sk,xi)
	 ...
	 if(apro.eq.1)then
	  do i=1,Nk
	   Sdek(i,1)=dble(i)*dy
	   Sdek(i,2)=SdkGCRPA(Sdek(i,1),parametro2,eps)
	  enddo
	 endif
	 if(apro.eq.2)then
c... Reads inpud data
...
c      
      x(1) = 1.0
      rmax = 80.0  ! rmax=800 para yukaws; rmax = 10-50 para corto alcance
c
      alfa = 0.5
      closure = 2.0
      nr   = 2**12
