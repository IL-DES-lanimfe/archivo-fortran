...
c      PARAMETER (nm=2**12)
	...
	dimension Sdek(2**12,2),Gder(2**12,2)
c      dimension r(nm),q(nm),g(nm,3),c(nm,3),s(nm,3)
c      COMMON /STRUKTUR/R,Q,G,C,S
c      common /syst/rho,sig(3),sigg(3),x(2),eps
c      COMMON /PARAMETERN/CLOSURE,ALFA,RMAX,QMAX,NR,NQ,EZ,MR,NRHO
c
	open(50,file='fundisradLJMRYfvd5nu14iso.dat')
	open(60,file='facdesLJMRYfvd5nu14iso.dat')
	d=0.954505040209140D0
	fv=0.5D0/d**3
c... Reads inpud data
      call input(fv,14.D0,0.35D0)
c... perform calculations
      call oz2(Sdek,Gder)
CC
	DO i=1,2**12
	 write(50,*)Gder(i,1),Gder(i,1)/d,Gder(i,2)
	 write(60,*)Sdek(i,1),Sdek(i,1)*d,Sdek(i,2)
	enddo
	close(50)
	close(60)
c... end of program
