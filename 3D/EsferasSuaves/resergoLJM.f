...
CC
	 ...
	 Double precision d,Gder(Nk,2),alfa
CC
...
CC
	 ...
	 write(*,*)'Estamos en el caso de LJM'
	 write(*,*)'parametro 1:fracc. de volumen,parametro 2:nu'
...
CC
	 ...
	 write(*,*)' '
       open(8,file='vidrioLJMRYDS.dat') 
       open(9,file='facnoergoLJMRYDS.dat')
       open(10,file='SkmaxLJMRYDS.dat')
CC
...
!	  par1post= par1max
!        par1ant= par1min
	  d=diametro(parametro2)
	  write(*,*)'d',d
CC
C pa que converga
CC
	  if (parametro2.ge.50)then
	   alfa=0.18D0
	   par1ant=0.565D0
	   par1post=0.56D0
	  endif
	  if (parametro2.eq.45)then
	   alfa=0.18D0
	   par1ant=0.565D0
	   par1post=0.575D0
	  endif
	  if ((parametro2.eq.40).or.(parametro2.eq.35))then
	   alfa=0.18D0
	   par1ant=0.57D0
	   par1post=0.575D0
	  endif
	  if (parametro2.eq.30)then
	   alfa=0.18D0
	   par1ant=0.58D0
	   par1post=0.585D0
	  endif
	  if (parametro2.eq.25)then
	   alfa=0.25D0
	   par1ant=0.59D0
	   par1post=0.595D0
	  endif
	  if (parametro2.eq.20)then
	   alfa=0.25D0
	   par1ant=0.6D0
	   par1post=0.61D0
	  endif
	  if (parametro2.eq.15)then
	   alfa=0.25D0
	   par1ant=0.625D0
	   par1post=0.63D0
	  endif
	  if (parametro2.le.10)then
	   alfa=8.D0
	   par1ant=4.3D0
	   par1post=4.4D0
	  endif
...
CC
c... Reads inpud data
      call input(par1ant,parametro2,alfa)
c... perform calculations
      call oz2(Sdek,Gder)
CC
C debemos poner todo unidades de esfera dura
CC
!	  do i=1,Nk
!		Sdek(i,1)=Sdek(i,1)/d
!	  enddo
...
CC
	  if (dabs(solant).lt.1.D-4)then
         write(8,*)sngl(par1ant),sngl(par1ant*d**3),
     &	         sngl(parametro2),sngl(gamam)
...
CC
c... Reads inpud data
      call input(par1post,parametro2,alfa)
c... perform calculations
      call oz2(Sdek,Gder)
CC
C debemos poner todo unidades de esfera dura
...
CC
	  if (dabs(solpost).lt.1.D-4)then
         write(8,*)sngl(par1post),sngl(par1post*d**3),sngl(parametro2),
     &	         integralpost,sngl(gamam)
...
CC
c... Reads inpud data
      call input(par1med,parametro2,alfa)
c... perform calculations
      call oz2(Sdek,Gder)
CC
C debemos poner todo unidades de esfera dura
...
CC
	   if (dabs(solmed).lt.1.D-3)then
          write(8,*)sngl(par1med),sngl(par1med*d**3),
     &	          sngl(parametro2),sngl(gamam)
...
c      
      x(1) = 1.0
      rmax = 100.0  ! rmax=800 para yukawas
