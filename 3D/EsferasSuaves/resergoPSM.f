...
C Enero 2007								C
C 									C
C Adapatado a esferas penetrables 					C
C									C
C Julio 2008								C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
CC
	 ...
	 write(*,*)' '
       open(8,file='vidrioPSM.dat') 
       open(9,file='facnoergoPSM.dat')
       open(10,file='SkmaxPSM.dat')
CC
...
CC
	 !z=10.D0
	 dy=1.5D-2
	 dpar2=1.D0 !(par2max-par2min)/dble(Npar2)
	 DO parametro2=par2min,par2max,dpar2
	  if(parametro2.eq.3.D0)then
	   par1post=4.2865D0
           par1ant= 4.2855D0
	  endif
	  if(parametro2.eq.4.D0)then
	   par1post=5.745D0
           par1ant= 5.744D0
	  endif
	  if(parametro2.eq.5.D0)then
	   par1post=7.205D0
           par1ant= 7.195D0
	  endif
	  if(parametro2.eq.6.D0)then
	   par1post=8.657D0
           par1ant= 8.656D0
	  endif
	  if(parametro2.eq.7.D0)then
	   par1post=10.112D0
           par1ant= 10.111D0
	  endif
	  if(parametro2.eq.8.D0)then
	   par1post=11.57D0
           par1ant= 11.56D0
	  endif
	  if(parametro2.eq.9.D0)then
	   par1post=13.019D0
           par1ant= 13.018D0
	  endif
	  if(parametro2.eq.10.D0)then
	   par1post=14.472D0
           par1ant= 14.471D0
	  endif
	  if(parametro2.eq.11.D0)then
	   par1post=15.925D0
           par1ant= 15.924D0
	  endif
	  if(parametro2.eq.12.D0)then
	   par1post=17.378D0
           par1ant=17.376D0
	  endif
	  if(parametro2.eq.13.D0)then
	   par1post=18.83D0
           par1ant=18.829D0
	  endif
	  if(parametro2.eq.14.D0)then
	   par1post=20.282D0
           par1ant=20.281D0
	  endif
