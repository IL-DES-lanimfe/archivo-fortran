...
C variables auxiliares
        Double precision pi,fv,z,xi,intmx,hues,Kah,fvh
...
C archivo de salida funcional criterio 
	open(10,file='solsyukz20.dat')
        open(20,file='funmaxsyukz20.dat')
	open(30,file='huesoyukz20.dat')
CC
        pi= dacos(-1.D0)
CC
...
CC
	z=20.D0
	DO fv=0.574,0.7,0.01
	 hues=100.D0
	 DO parametro2=1.D0,20.D0,0.05
         write(*,*)'parametros',fv,parametro2
CC
...
CC
	 call msa(fv,parametro2,z,0.01D0,nk,Rk,Sk,xi)
	 do i=1,nk
	  Sdek(i,1)=2.D0*Rk(i)
	  Sdek(i,2)=Sk(i)
...
CC
	   ...
	   y=Sdek(iint,1)
           S=SdeK(iint,2)
           l= lambda(y,kmin)
           num= dzeta*((S-1.D0)*l*y**2)**2
           den= 36.D0*pi*fv*(dzeta*y**2+S*l)*(dzeta*y**2+l)
           T(iint)= num/den
          enddo
CC
...
CC
	  ...
	  if(integral.gt.intmx)intmx=integral
...
CC
	 ...
	 write(20,*)fv,parametro2,intmx
CC Termina el ciclo de evaluacion de la funcional
C
CC Calculamos el minmo para ver cual es el hueso
C
	 if(intmx.lt.hues)then
	  hues=intmx
	  fvh=fv
	  Kah=parametro2
	 endif 
