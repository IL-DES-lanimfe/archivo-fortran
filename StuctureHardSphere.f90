...
!       read(*,*)dec
	 ...
	 dec='s'
! CC
! C Verlet-Weis correction
! CC
	 fv=fv-fv**2/16.D0
       call Sdeked(Sdek,Gder,fv,Nk,1000.D0,10.D0,dec)
       open(10,file="SkHSfv0,5.dat")
       do i=1,Nk
        write(10,*)Sdek(i,1),Sdek(i,2)
       enddo
       close(10)
       if(dec.eq.'s')then
        open(11,file='grHSfv0,5.dat')
        do i=1,Nk
...
!       DIMENSION XKR(NK),XIN(NK)
      integer :: N,LL,NK,JJ,J,K
      double precision :: PI,fv,rmx,DENS,KMAX,DRK,RMAX,DR,RES1
      Double precision :: SDK(NK,2),GDR(N,2)
      Double precision :: R(N),HR(N),RK(NK),HK(NK)
      Double precision :: XKR(NK),XIN(NK)
...
! 	IMPLICIT DOUBLE PRECISION(A-H,O-Z)
	Integer :: M, I
	double precision :: DR,RES1,S0,S1,S2
        Double precision :: XIN(M)
	...
	IF(MOD(M,2).EQ.0) RES1=RES1+DR*(5.D0*XIN(M)+8.D0*XIN(M-1)-XIN(M-2))/12.D0
	RETURN
	END
