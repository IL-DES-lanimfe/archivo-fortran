...
C auxiliares
      ...
      DIMENSION Sdek(NK,2),Gder(NK,2),Yder(NK,2),GG0(NK)
      PARAMETER (XNU=50.D0)
      PARAMETER (EUL=0.5572D0)
	
C      OPEN (1,file='grss17p.dat')
C      open (2,file='skss17p.dat')
      OPEN (15,FILE='fktmaxbrincom.dat')
      OPEN (16,FILE='deltazbrincom.dat')
      OPEN (17,FILE='fktbrincom.dat')

CC Mensaje inicial

      write(*,*)' '
      write(*,*)'Estas echando a andar el metodo de Fuchs para resolver'
...
CCCCCCCCCCC FACTOR DE ESTRUCTURA Y CANTIDAES ESTATICAS CCCCCCCCCCCCCCCC

CC
C reescalamiento en fraccion de volumen para hacer coicidir S(k) y g(r)
C con las subrutinas de momentos
CC 
C	XLA=1.D0+EUL/XNU
C	PHIE=PHI/XLA**3
CC
...
CC
C	do IK=1,NK
C	 read(1,*)Gder(IK,1),Gder(IK,2)
C	 read(2,*)Sdek(IK,1),Sdek(IK,2)
C	enddo
C	close(1)
C	close(2)
       call Sdekes(Sdek,Gder,Yder,PHIE,NK,100.D0,20.D0,XNU)
	write(*,*)'ya esta S(k), g(r) y Y(r)',PHI

CC minimo de S(k) para lambda
...
CC
	...
	write(*,*)'ya esta As(k) y Bs(r)',PHI
CC
C inicia el calculo de los coeficientes Ac y Bc
CC
       call COLMOM(PHI,XNU,1.D0,Sdek,NK,Gder,NK-1,AT,BT)
	write(*,*)'ya esta Ac(k) y Bc(r)',PHI
 ...
 !     READ(13,*)T(IT)!,DELTAZ(IT)
      ...
      DO IK=1,NK
	   ...
	   CC(IK,IT)=CCSEXP(IK,IT)
         G(IK,IT)=CCSEXP(IK,IT)/CSSEXP(IK,IT)
         DFS(IK,IT)=FS(IK,IT)
         DXC(IK,IT)=XC(IK,IT)
         DCS(IK,IT)=CS(IK,IT)
         DCC(IK,IT)=CC(IK,IT)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc	   
     ...
     ^                   +(1./H + DCC(IK,1))*XC(IK,IT-1)+SUMAC
	      ...
	      GRANDO=GRANDO+GG(IK)*FS(IK,IT)*XC(IK,IT)
            ...
            !write(*,*)grando,FS(383,IT),XC(383,IT)
		!CRITERIO DE CONVERGENCIA	 		
		DELTAZ(IT)=GRANDO*DY/(36.*PHI*PI)
            GRANDO=0.
            !write(*,*)deltaz(it),deltazold(it)
            !WRITE(*,*)FS(100,IT),XC(100,IT)
		IF(ABS(DELTAZ(IT)-DELTAZOLD(IT)).LT.TOL) THEN
c                  grando2(ik)=convol
		...
		stop
            ...
            !TERMINA EL CRITERIO DE CONVERGENCIA
...
C      DO IT=1,MT/2
C      DELTAZ(IT)=DELTAZ(2*IT)
C      T(IT)=T(2*IT)
C       DO IK=1,NK
C       FS(IK,IT)=FS(IK,2*IT)
C       XC(IK,IT)=XC(IK,2*IT)
C       CS(IK,IT)=CS(IK,2*IT)
C       CC(IK,IT)=CC(IK,2*IT)
C       DFS(IK,IT)=.5*(DFS(IK,2*IT)+DFS(IK,2*IT))
C       DXC(IK,IT)=.5*(DXC(IK,2*IT)+DXC(IK,2*IT))
C       DCS(IK,IT)=.5*(DCS(IK,2*IT)+DCS(IK,2*IT))
C       DCC(IK,IT)=.5*(DCC(IK,2*IT)+DCC(IK,2*IT))
C       ENDDO
C      ENDDO


C       DO IT=MT/2+1,MT,1  !COMIENZA INTERVALO TEMPORAL
C         T(IT)=T(IT-1)+DT
CINPUT INICIAL DE DELTAZ
	 
C         DELTAZ(IT)=2.*DELTAZ(IT-1)-DELTAZ(IT-2) 
         
C          DO IA=1,NITER         !CICLO DE ITERACIONES        
            
C            DELTAZOLD(IT)=DELTAZ(IT)

C            GRANDO=0.
C            DO IK=1,NK         !CICLO SOBRE VECTORES DE ONDA
C             CSSEXP(IK,IT)=AS(IK)*EXP(-BS(IK)*T(IT))
C             CCSEXP(IK,IT)=AT(IK)*EXP(-BT(IK)*T(IT))
C              
C              CS(IK,IT)=CSSEXP(IK,IT)
C     ^                  +(DELTAZ(IT)-CSSEXP(IK,IT))*XLAM(IK)
c              if(IT.EQ.184)WRITE(*,*)'sexp',CSSEXP(IK,IT),CCSEXP(IK,IT)
c              if(IT.EQ.184)WRITE(*,*)'self',CS(IK,IT),DELTAZ(IT)
c              convol=grando2(ik)+CS(IK,IT)exp(-BT(ik)*T(it))*H     !ojo con los reescal
CCCCCCC   vineyard CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C	    IF(LVINE.EQ.1)THEN
C              CONVOL=0.
C               DO ITT=1,IT
C               CONVOL=CONVOL+DELTAZ(ITT)*EXP(BT(IK)*(T(ITT)-T(IT)))
C               ENDDO
C              CC(IK,IT)=CS(IK,IT)+
C     ^      (1.-XLAM(IK))*(AS(IK)*CCSEXP(IK,IT)/AT(IK)-CSSEXP(IK,IT))+
C     ^      (BS(IK)-BT(IK))*XLAM(IK)*CONVOL*DT
C		  CC(IK,IT)=AT(ik)*CC(IK,IT)/AS(IK)
C	    ELSE
C	      CC(IK,IT)=CS(IK,IT)+CCSEXP(IK,IT)-CSSEXP(IK,IT)
...
C              DO IS=2,IT/2    !CICLO DE SUMATORIA
C              SUMAS=SUMAS+DFS(IK,IS)*(CS(IK,IT-IS)-CS(IK,IT-IS+1))
C     ^                   +DCS(IK,IS)*(FS(IK,IT-IS)-FS(IK,IT-IS+1))
C              SUMAC=SUMAC+DXC(IK,IS)*(CC(IK,IT-IS)-CC(IK,IT-IS+1))
C     ^                   +DCC(IK,IS)*(XC(IK,IT-IS)-XC(IK,IT-IS+1))
C              ENDDO           !TERMINA CICLO DE SUMATORIA
C              ASS(IT)=-CS(IK,IT/2)*FS(IK,IT/2)+DFS(IK,1)*CS(IK,IT-1)
C     ^                   +(1./H + DCS(IK,1))*FS(IK,IT-1)+SUMAS
C              DEN1=(1./H+Y(IK)**2+DCS(IK,1))
C      	  FS(IK,IT)=(CS(IK,IT)*(1.-DFS(IK,1))+ASS(IT))/DEN1
C              A(IT)=-CC(IK,IT/2)*XC(IK,IT/2)+DXC(IK,1)*CC(IK,IT-1)
C     ^                   +(1./H + DCC(IK,1))*XC(IK,IT-1)+SUMAC
C              DEN2=(1./H+(Y(IK)**2)/S(IK)+DCC(IK,1))
C		  XC(IK,IT)=(CC(IK,IT)*(1.-DXC(IK,1))+A(IT))/DEN2
C              ELSE
C              DO IS=2,IT/2    !CICLO DE SUMATORIA
C              SUMAS=SUMAS+DFS(IK,IS)*(CS(IK,IT-IS)-CS(IK,IT-IS+1))
C     ^                   +DCS(IK,IS)*(FS(IK,IT-IS)-FS(IK,IT-IS+1))
...
C     ^                   +DCC(IK,IS)*(XC(IK,IT-IS)-XC(IK,IT-IS+1))
C              ENDDO           !TERMINA CICLO DE SUMATORIA
C           SUMAS=SUMAS+DFS(IK,IT-IT/2)*(CS(IK,IT-IT/2)-CS(IK,IT/2))/2.
C     ^             +DCS(IK,IT-IT/2)*(FS(IK,IT-IT/2)-FS(IK,IT/2))/2.
C           SUMAC=SUMAC+DXC(IK,IT-IT/2)*(CC(IK,IT-IT/2)-CC(IK,IT/2))/2.
C     ^             +DCC(IK,IT-IT/2)*(XC(IK,IT-IT/2)-XC(IK,IT/2))/2.
C      PROM=(CS(IK,IT-IT/2)+CS(IK,IT/2))*(FS(IK,IT-IT/2)+FS(IK,IT/2))/4.C

C              ASS(IT)=-PROM+DFS(IK,1)*CS(IK,IT-1)
C     ^                   +(1./H + DCS(IK,1))*FS(IK,IT-1)+SUMAS
C              DEN1=(1./H+Y(IK)**2+DCS(IK,1))
C      	  FS(IK,IT)=(CS(IK,IT)*(1.-DFS(IK,1))+ASS(IT))/DEN1
C      PROM=(CC(IK,IT-IT/2)+CC(IK,IT/2))*(XC(IK,IT-IT/2)+XC(IK,IT/2))/4.
C              A(IT)=-PROM+DXC(IK,1)*CC(IK,IT-1)
C     ^                   +(1./H + DCC(IK,1))*XC(IK,IT-1)+SUMAC
C              DEN2=(1./H+(Y(IK)**2)/S(IK)+DCC(IK,1))
C		  XC(IK,IT)=(CC(IK,IT)*(1.-DXC(IK,1))+A(IT))/DEN2
C              ENDIF
c              if(IT.EQ.184)WRITE(*,*)'sum',SUMAS,SUMAC,ASS(IT),A(IT)
C              DFS(IK,IT)=FS(IK,IT)
C              DXC(IK,IT)=XC(IK,IT)
C              DCS(IK,IT)=CS(IK,IT)
C              DCC(IK,IT)=CC(IK,IT)
C	      GRANDO=GRANDO+GG(IK)*FS(IK,IT)*XC(IK,IT)
c              if(IT.EQ.184)WRITE(*,*)'grando',GRANDO
C            ENDDO             !CICLO SOBRE VECTORES DE ONDA
            !write(*,*)grando,FS(383,IT),XC(383,IT)
		!CRITERIO DE CONVERGENCIA	 		
C		DELTAZ(IT)=GRANDO*DY/(36.*PHI*PI)
C            GRANDO=0.
c            write(*,*)it,ia,t(it),deltazold(it),deltaz(it)
            !WRITE(*,*)FS(100,IT),XC(100,IT)
C		IF(ABS(DELTAZ(IT)-DELTAZOLD(IT)).LT.TOL) THEN
c                  grando2(ik)=convol
C                  if(mod(it,100).eq.0.)WRITE(*,*)"ad",t(it)
C                  GOTO 2006
C            ENDIF
C            IF(IA.EQ.NITER) then
C            WRITE(*,*) "NO CONVIRGIO",T(IT)
C		stop
C            endif
            !TERMINA EL CRITERIO DE CONVERGENCIA
C          ENDDO              !CICLO DE ITERACIONES         
C 2006  continue		
C       ENDDO                !TERMINA INTERVALO TEMPORAL

C       write(*,*)ires,t(mt)
C      IF(IRES.LE.NRES)GOTO 201
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      SUBROUTINE COLMOM(PHI,XZK,A,SK,NL,GR,N,AC,BC)
...
C ARREGLOS QUE SE NECESITAN PARA MODIFICAR A SUBRUTINA
      DIMENSION SK(NL,2),GR(NL,2),AC(NL),BC(NL)
