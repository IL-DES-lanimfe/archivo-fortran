...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	...
	Integer i,Ndk,nsteps,dec,ikmax,ikshow,Lmaximo
	parameter(Ndk=2**10)
	...
	Double precision Tp,fvp,delT,deltem,st
...
CC
	...
	call msa(fv,Ka,20.D0,0.01D0,Ndk,Rk,Sk,chhi)
	do i=1,Ndk
	 Sdeki(i,1)=2.D0*Rk(i)
	 Sdeki(i,2)=Sk(i)
	enddo
...
CC
	if (dec.eq.1) then
	 open(20,file='relaxk10taus400g.dat')
	 ikshow=10
	endif
	if (dec.eq.2)then
	 open(20,file='relaxCkmaxtaus400g.dat')
	 ikshow=ikmax
	endif
...
CC
	if(dec.eq.1)then
	 Tp=Ti+delT
	 open(30,file='relax2pasosk10taus400g.dat')
	endif
	if(dec.eq.2)then
	 fvp=fvi+delT
	 open(30,file='relaxC2pasoskmaxtaus400g.dat')
	endif 
CC
...
CC
	  ...
	  call SCGLE(Sdeki,fv,delzfin)
CC
C Calculo de coeficiente de difusion a tiempos largos
CC
! 	  open(15,file='delz.dat')
! 	  do i=1,7000
! 	   read(15,*)delzfin(i,1),delzfin(i,2)
! 	  enddo
! 	  close(15)
	  D=DLT(delzfin)
CC
...
C									C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      Subroutine  SCGLE(Sdek,fv,deltazf)
...
CC Mensaje inicial
       	write(*,*)' '
       	write(*,*)'resolviendo el esquema autoconsistente'
       	write(*,*)' '
! 	open(26,file='delz.dat')
       	PI=4.*ATAN(1.0)
       	IRES=0
C     DATOS DE ENTRADA:
	PHI=fv 
