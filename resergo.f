...
CC
	...
	Double precision fv
	Double precision dzetamin,dzetamax,ddzeta
CC
...
CC
	Integer iint
	Double precision y,dy,S,g,l,dzeta
...
CC
	Double precision Gder(Nk,2),Yder(Nk,2)
CC
...
CC
	...
	write(*,*)'Estamos en el caso de (1/r)**n'
	write(*,*)'parametro1=frac. de vol.,parametro2=n'
...
CC
	...
	dpar2=dabs(par2max-par2min)/dble(Npar2)
        ...
        open(10,file='S(k)maxre.dat')
C        DO parametro2=par2min, par2max,dpar2
CC
...
CC
         call Sdekes(Sdek,Gder,Yder,par1ant,Nk,100.D0,10.D0,nu)
...
CC
         if (dabs(solant).lt.1.D-4)then
          write(8,*)par1ant,parametro2,integralant,dzeta
          write(*,*)'se encontro solucion para: ',par1ant,dzeta
          write(*,*)'evaluando factor no ergodico'
          call facnoerg(dzeta,Sdek,g0,kmin,Nk,fnoer)
          do i=1,Nk
...
C ponga aqui la subrutina de S(k) evaluandola en par1post y parametro2
	 call Sdekes(Sdek,Gder,Yder,par1post,Nk,100.D0,10.D0,nu)
	 dy=Sdek(2,1)-Sdek(1,1)
	 write(*,*)'calculando k del primer minimo de S(k)'
	 kmin=yminima(Sdek,Nk)
	 write(*,*)'calculando la funcion g0(k)'
...
CC
         if (dabs(solpost).lt.1.D-4)then
          write(8,*)par1post,parametro2,integralpost,dzeta
          write(*,*)'se encontro solucion para: ',par1post,dzeta
          write(*,*)'evaluando factor no ergodico'
          call facnoerg(dzeta,Sdek,g0,kmin,Nk,fnoer)
          do i=1,Nk
           write(9,*)fnoer(i,1),fnoer(i,2)
...
C llame aqui a la subrutina de S(k) evaluandola en par1med y parametro2
	  call Sdekes(Sdek,Gder,Yder,par1med,Nk,100.D0,10.D0,nu)
	  dy=Sdek(2,1)-Sdek(1,1)
	  write(*,*)'calculando k del primer minimo de S(k)'
	  kmin=yminima(Sdek,Nk)
	  write(*,*)'calculando la funcion g0(k)'
...
CC
         if (dabs(solmed).lt.1.D-4)then
          write(8,*)par1med,parametro2,integralmed,dzeta
          write(*,*)'se encontro solucion para: ',par1med,dzeta
          write(*,*)'evaluando factor no ergodico'
          call facnoerg(dzeta,Sdek,g0,kmin,Nk,fnoer)
          do i=1,Nk
           write(9,*)fnoer(i,1),fnoer(i,2)
...
100	CONTINUE
	...
	END
       ...
       Subroutine  facnoerg(deltaz,Sk,gcero,kmin,nk,f)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C								     C
