...
C auxiliares
       ...
       DIMENSION DEZAUX(20000),TAUX(20000)
!       OPEN (12,FILE='grsspy.dat')
       OPEN (13,FILE='facdesLJMfv536nu50.dat')
       OPEN (14,FILE='selfLJMnu50fv536.dat')	
       OPEN (15,FILE='colLJMnu50fv536.dat')
!       OPEN (50,FILE='hdk.dat')
       OPEN (25,FILE='fktmaxLJMnu50fv536.dat')
       OPEN (26,FILE='deltazLJMnu50fv536.dat')
       OPEN (27,FILE='fktLJMnu50fv536.dat')
       OPEN (28,FILE='sueltosLJMnu50fv536.dat')
CC Mensaje inicial
       write(*,*)' '
...
cc ARREGLO PARA LA CONVOLUCION
	 DEZAUX(IT)=DELTAZ(IT)
...
CCCCCCC   vineyard CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	   IF(LVINE.EQ.1)THEN
             CONVOL=0.
            DO ITT=1,IT+IRES*MT/2
	     DTAUX=TAUX(ITT)-TAUX(ITT-1)
          CONVOL=CONVOL+DEZAUX(ITT)*EXP(BT(IK)*(TAUX(ITT)-T(IT)))*DTAUX
            ENDDO
            CC(IK,IT)=CS(IK,IT)+
     ^      (1.-XLAM(IK))*(AS(IK)*CCSEXP(IK,IT)/AT(IK)-CSSEXP(IK,IT))+
     ^      (BS(IK)-BT(IK))*XLAM(IK)*CONVOL
	    CC(IK,IT)=AT(ik)*CC(IK,IT)/AS(IK)
	   ELSE
            CC(IK,IT)=CS(IK,IT)+CCSEXP(IK,IT)-CSSEXP(IK,IT)
	   ENDIF
...
!FIN CRITERIO DE CONVERGENCIA
	...
	DEZAUX(IT+IRES*MT/2)=DELTAZ(IT)
        ENDDO              !CICLO DE ITERACIONES         
	TAUX(IT+IRES*MT/2)=T(IT)
 2006  continue		
       ENDDO                !TERMINA INTERVALO TEMPORAL
       ...
       DO IT=1,MT+IRES*MT/2
	WRITE(50,*)TAUX(IT),DEZAUX(IT)
!         DO IK=1,NK
!          WRITE(27,*)Y(IK),sngl(XC(IK,IT)*S(IK)),sngl(XC(IK,IT))
