...
C								     C
C Este programa sirve para calcular la curva spinodal. Como se traba-C
C ja el caso de Yukawas atractivos, la spinodal se ubica cuando la   C
C compresibilidad se hace infinita.				     C
C								     C
C Enero 2007							     C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
	implicit none !toda variable se tiene que decalrar
	Double precision z,Ka,fv,dKa
	Double precision fvmin,fvmax,dfv
	Double precision Rk(2**12),Sk(2**12)
	Double precision xi,spin
CC
C parametros fijos
CC
	z=12.5D0
	Ka=0.001D0 !solo el valor inicial
	dKa=1.D0  !solo el valor inicial
	fvmin=0.3D0
	fvmax=0.6D0
	dfv=0.01D0
CC
C archivo de salida
CC
	open(50,file='spinodal.dat')
CC
C iniciamos el ciclo para barrer en fracciones de volumen
CC
	DO fv=fvmin,fvmax,dfv	
 200	 continue
	 write(*,*)'calculando S(k)',fv,Ka
CC
C Primero calculamos el factor de estructura
CC
	 call msa(fv,Ka,z,0.001D0,2**12,Rk,Sk,xi,spin)
CC
C para ir monitoreando
CC
	 write(*,*)' '
	 write(*,*)'p',fv,Ka,spin
	 write(*,*)' '
CC
C Ahora vemos si el parametro spin se hace cero
CC
	if(dabs(spin).LE.1.D-4) then
	 write(50,*)fv,1.D0/Ka,Ka
	 write(*,*)'se encontró la spinodal para:',fv,Ka
	 goto 100
	endif
CC
C si no, vemos si ya se hizo negativo para regresarnos
CC
	if(spin.LT.0.D0) then
	 dKa=dKa/10.D0
	 Ka=Ka-10.D0*dKa+dKa
	 goto 200
	endif
CC
C Si no se cumplen las dos anteriores seguimos bajando en temperatura
CC
	 Ka=Ka+dKa
	 goto 200
CC
